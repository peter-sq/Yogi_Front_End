import Images from "../images";

export const items = [
    {
      image: Images.send,
      title: 'Send & Receive Easily',
      description: 'Send and Recieve Crypto Assets Efficiently with Yogi',
    },
    {
      image: Images.invest,
      title: 'Invest in Real Time',
      description: 'Invest Live Without Disruption and interuption from 3rd parties.',
    },
    {
      image: Images.graph,
      title: 'Watch and Analyze Chats',
      description: 'Simple and Accessible Layout for Monitoring Your Trading.',
    },
  ];


  export const footerNavs = [
    {
        label: "Resources",
        items: [
            {
                href: 'javascript:void()',
                name: 'contact'
            },
            {
                href: 'javascript:void()',
                name: 'Support'
            },
           
            {
                href: 'javascript:void()',
                name: 'Pricing'
            },
        ],
    },
    {
        label: "About",
        items: [
            {
                href: 'javascript:void()',
                name: 'Terms'
            },
            {
                href: 'javascript:void()',
                name: 'License'
            },
            {
                href: 'javascript:void()',
                name: 'Privacy'
            },
            {
                href: 'javascript:void()',
                name: 'About US'
            },
        ]
    },
    {
        label: "Explore",
        items: [
            {
                href: 'javascript:void()',
                name: 'Calculator'
            },
          
            {
                href: 'javascript:void()',
                name: 'Blog'
            },
        ]
    },
    {
        label: "Company",
        items: [
            {
                href: 'javascript:void()',
                name: 'Partners'
            },
            {
                href: 'javascript:void()',
                name: 'Team'
            },
            {
                href: 'javascript:void()',
                name: 'Careers'
            },
        ],
    }
]

  export const faqData = {
    "Account Management": [
      {
        id: "am1",
        question: "How do I reset my password?",
        answer: "To reset your password, click on the 'Forgot Password' link on the login page. Enter your email address, and we'll send you instructions to reset your password."
      },
      {
        id: "am2",
        question: "How can I update my profile information?",
        answer: "Navigate to your account settings, click on 'Edit Profile', make your desired changes, and click 'Save' to update your information."
      },
      {
        id: "am3",
        question: "What should I do if I can't log in?",
        answer: "First, verify your credentials and ensure caps lock is off. If issues persist, try clearing your browser cache or use the password reset option."
      }
    ],
    "Billing": [
      {
        id: "b1",
        question: "How do I view my current billing status?",
        answer: "Access your billing dashboard through your account settings. Here you'll find your current plan, payment history, and upcoming charges."
      },
      {
        id: "b2",
        question: "What payment methods are accepted?",
        answer: "We accept all major credit cards (Visa, MasterCard, American Express), PayPal, and bank transfers for business accounts."
      },
      {
        id: "b3",
        question: "How can I update my payment information?",
        answer: "Go to 'Billing Settings' in your account, select 'Payment Methods', and click 'Add New' or 'Edit' existing payment information."
      }
    ],
    "Troubleshooting": [
      {
        id: "t1",
        question: "What should I do if I encounter a technical issue?",
        answer: "First, try refreshing your browser and clearing cache. If the issue persists, check our system status page or contact technical support."
      },
      {
        id: "t2",
        question: "How do I contact technical support?",
        answer: "You can reach our technical support team 24/7 via live chat, email at support@example.com, or call us at 1-800-SUPPORT."
      },
      {
        id: "t3",
        question: "Are there known system outages?",
        answer: "Check our real-time system status dashboard for any known issues or planned maintenance schedules."
      }
    ]
  };